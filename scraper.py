...
file_handler = logging.FileHandler('bible_scraper.log', encoding='utf-8')
file_handler.setLevel(logging.INFO)
file_formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
file_handler.setFormatter(file_formatter)
logger.addHandler(file_handler)

# Console handler (warnings and errors only to avoid cluttering progress bar)
...
class BibleScraper:
    ...
    def draw_progress_bar(self, completed: int, total: int, current_book: str, current_chapter: int, current_verse: int, session_start_time: float, session_completed: int, translations_count: int) -> None:
        """Draw a beautiful live-updating console progress bar."""
        # Calculate percentage
        pct = (completed / total) * 100 if total > 0 else 0
        
        # Calculate speed and ETA based on active scraping session
        elapsed = time.time() - session_start_time
        speed = session_completed / elapsed if elapsed > 0 else 0
        if speed > 0:
        ...
        # Create visual progress bar block
        bar_width = 25
        filled_width = int(pct / 100 * bar_width)
        bar = "█" * filled_width + "░" * (bar_width - filled_width)
        
        # Format stats line (limit max length to fit comfortably in terminal)
        status_line = (
            f"\r[{bar}] {pct:>5.1f}% | "
            ...
            f"{speed:.1f} v/s | "
            f"ETA: {eta_str} | "
            f"Translations: {translations_count}"
        )
    ...
    def scrape_all(self) -> None:
        ...
        # Calculate total verses in canonical structure
        total_verses = 0
        ...
        print(f"\n🚀 Starting scrape of {len(BIBLE_STRUCTURE)} books (~{total_verses:,} total verses)")
        if completed > 0:
            print(f"🔄 Resuming from {completed:,} completed verses ({completed/total_verses*100:.1f}%)")
        print(f"📝 Logging detailed output to 'bible_scraper.log'")
        ...
        for book, chapters_data in BIBLE_STRUCTURE.items():
            ...
            # Print a neat transition when starting a new book (with clear newlines)
            book_display = book.replace('_', ' ')
            sys.stdout.write(f"\n📖 [Book] {book_display} ({num_chapters} chapters, {sum(verses_per_chapter)} verses)...\n")
            sys.stdout.flush()
            ...
            for chapter in range(1, num_chapters + 1):
                ...
                for verse in range(1, num_verses + 1):
                    # Check if already completed
                    is_done = self.is_verse_completed(book, chapter, verse)
                    ...
                    if not is_done:
                        ...
                        if success:
                            ...
                            # Keep logging failure details
                            logger.error(f"Failed to scrape verse: {book} {chapter}:{verse}")
        ...
        # Done! Finalize progress line and print final stats
        sys.stdout.write("\n\n🎉 Scraping completed!\n")
        sys.stdout.flush()
        ...
        logger.info(f"Total verses verified/scraped: {global_completed}")
...
def main():
    ...
    if not args.resume and not args.overwrite:
        if Path(args.output).exists() or Path(progress_file).exists():
            ...
            print(f"    - Output file:   {args.output} (exists: {Path(args.output).exists()})")
            print(f"    - Progress file: {progress_file} (exists: {Path(progress_file).exists()})")
            print("\n    Running without '--resume' will completely OVERWRITE and DELETE all progress!")
            print("!" * 80 + "\n")
            
            # Interactive prompt if standard input is a terminal
            if sys.stdin.isatty():
                try:
                    choice = input("Would you like to:\n  [r] Resume previous scrape (highly recommended)\n  [o] Overwrite and start completely fresh\n  [a] Abort\nChoose (r/o/a) [default: r]: ").strip().lower()
                    if choice == 'o':
                        print("⚠️   Starting fresh. Existing files will be overwritten.")
                    elif choice == 'a':
                        print("❌ Aborted by user.")
                        sys.exit(0)
                    else:
                        print("🔄 Resuming previous scrape...")
                        args.resume = True
                except (KeyboardInterrupt, EOFError):
                    print("\n❌ Aborted.")
                    sys.exit(0)
            else:
                # Non-interactive fallback: exit safely with directions
                print("❌ ERROR: Existing data files found. To prevent accidental data loss, you must run with:")
                print("   --resume      to continue previous scrape, or")
                print("   --overwrite   to force starting from scratch.")
                sys.exit(1)
